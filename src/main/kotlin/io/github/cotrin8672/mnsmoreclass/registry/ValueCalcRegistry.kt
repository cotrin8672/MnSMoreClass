...
object ValueCalcRegistry : IMnsRegistry<ValueCalculation> by MnsRegistryDelegate() {
    ...
    val Shuriken: ValueCalculation =
            ValueCalcBuilderKt.of(NinjaSpells.SHURIKEN).spellScaling(0.6f, 1.2f).build()
    ...
    val ShadowSecret: ValueCalculation =
            ValueCalcBuilderKt.of(NinjaSpells.SHADOW_SECRET).attackScaling(0.0f, 0.0f).build()

    val WeaknessAura: ValueCalculation = ValueCalcBuilderKt.of(SpellRegistry.WEAKNESS_AURA).build()

    val EntanglingThornsDot: ValueCalculation =
            ValueCalcBuilderKt.of(SpellRegistry.ENTANGLING_THORNS)
                    .attackScaling(0.45f, 1.1f)
                    .capScaling(0.75f)
                    .build()
    ...
    fun register() {
        ...
        ValueCalcBuilderKt.of(SpellRegistry.PURIFICATION).build().add()
